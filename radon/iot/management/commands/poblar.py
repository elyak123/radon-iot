...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        if settings.DEBUG:
            ...
            pedidos_por_usuario = 20
            vechiculos_por_gasera = 12
            rutas_por_gasera = (pedidos_por_usuario * consumidores_por_gasera) / 30
            tz = pytz.timezone(settings.TIME_ZONE)
            gaseras = user_factories.GaseraFactory.create_batch(cantidad_gaseras)
            for index, gasera in enumerate(gaseras):
                usrs = user_factories.UserFactory.create_batch(consumidores_por_gasera, gasera=gasera, tipo='CONSUMIDOR')
                ...
                rutas_factories.VehiculoFactory.create_batch(vechiculos_por_gasera, gasera=gasera)
        else:
